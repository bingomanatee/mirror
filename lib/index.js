(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    const TYPE_ARRAY = Symbol('type:array');
    const TYPE_MIRROR = Symbol('type:mirror');
    const NAME_UNNAMED = Symbol('unnamed');

    const ABSENT = Symbol('ABSENT');

    const EVENT_TYPE_NEXT = Symbol('next');
    const EVENT_TYPE_SET = Symbol('set');
    const EVENT_TYPE_DEBUG = Symbol('debug');
    const EVENT_TYPE_CLEAN = Symbol('clean');

    const EVENT_TYPE_ACCEPT_FROM = Symbol('event:acceptFrom');
    const EVENT_TYPE_REMOVE_FROM = Symbol('event:removeFrom');

    const EVENT_TYPE_FLUSH_ACTIVE = Symbol('event:commit');
    const EVENT_TYPE_ACTION = Symbol('event:action');
    const EVENT_TYPE_MUTATE = Symbol('event:mutate');
    const EVENT_TYPE_VALIDATE = Symbol('event:validate');
    const SET_RE = /^set(.+)$/i;

    const MIRROR_EVENT_STATE_ACTIVE = Symbol('mirror-event-state:active');
    const MIRROR_EVENT_STATE_ERROR = Symbol('mirror-state:error');
    const MIRROR_EVENT_STATE_STOPPED = Symbol('mirror-state:stopped');
    const MIRROR_EVENT_STATE_COMPLETE = Symbol('mirror-state:complete');
    ...
    var constants = /*#__PURE__*/Object.freeze({
        ...
        TYPE_MIRROR: TYPE_MIRROR,
        NAME_UNNAMED: NAME_UNNAMED,
        ABSENT: ABSENT,
        EVENT_TYPE_NEXT: EVENT_TYPE_NEXT,
        EVENT_TYPE_SET: EVENT_TYPE_SET,
        EVENT_TYPE_DEBUG: EVENT_TYPE_DEBUG,
        EVENT_TYPE_CLEAN: EVENT_TYPE_CLEAN,
        EVENT_TYPE_ACCEPT_FROM: EVENT_TYPE_ACCEPT_FROM,
        EVENT_TYPE_REMOVE_FROM: EVENT_TYPE_REMOVE_FROM,
        EVENT_TYPE_FLUSH_ACTIVE: EVENT_TYPE_FLUSH_ACTIVE,
        EVENT_TYPE_ACTION: EVENT_TYPE_ACTION,
        EVENT_TYPE_MUTATE: EVENT_TYPE_MUTATE,
        EVENT_TYPE_VALIDATE: EVENT_TYPE_VALIDATE,
        SET_RE: SET_RE,
        MIRROR_EVENT_STATE_ACTIVE: MIRROR_EVENT_STATE_ACTIVE,
        MIRROR_EVENT_STATE_ERROR: MIRROR_EVENT_STATE_ERROR,
        MIRROR_EVENT_STATE_STOPPED: MIRROR_EVENT_STATE_STOPPED,
        MIRROR_EVENT_STATE_COMPLETE: MIRROR_EVENT_STATE_COMPLETE
    });
    ...
    const isMirror = (target) => target instanceof Mirror;
    const create = (...args) => new Mirror(...args);

    const isNum = require('lodash/isNumber');

    const isNumber = isNum;
    ...
    function hasKey(value, key, vType = null) {
      if (!vType) {
    ...
    function amend(value, values, type = ABSENT) {
      if (!isThere(type)) {
    ...
    function clone(value, type = ABSENT) {
      if (!isThere(type)) {
        type = typeOfValue(value);
      }
      let out = value;
    ...
    function getKey(value, key, vType = null) {
      if (!vType) {
        vType = typeOfValue(value);
      }
    ...
    function setKey(value, key, childValue, vType = null) {
      if (!vType) {
        vType = typeOfValue(value);
      }
    ...
    var eventMixin = (BaseClass) => class WithEvents extends BaseClass {
      constructor(...args) {
        super(...args);
    ...
    var actionMixin = (BaseClass) => class WithActions extends BaseClass {
      ...
      $mutate(fn$1) {
        if (!isFn(fn$1)) {
          throw e('$mutate expects function', { source: '$mutate', fn: fn$1 });
        }

        if (this.$_mutable) {
          return this.next(fn$1(clone(this.value)));
        }

        let next = this.value;

        try {
          next = fn(this.value, fn$1);
        } catch (err) {
          next = this.next(fn$1(clone(this.value)));
        }
    ...
    var propsMixin = (BaseClass) => class WithProps extends BaseClass {
    ...
    var childMixin = (BaseClass) => class WithChildren extends BaseClass {
        ...
        this.$on(EVENT_TYPE_NEXT, (nextValue, evt, target) => {
          ...
          target.$children.forEach((child, key) => {
            ...
            if (hasKey(nextValue, key, valueType)) {
              ...
              if (child.value === childValue || isEqual_1(child.value, childValue)) {
                return;
              }
            ...
            if (target.$lastChange) {
              ...
              if (lastValueChildValue !== value) {
                try {
                  ...
                  if (target.$_mutable) {
                    ...
                    target.$lastChange.value = fn(target.$lastChange.value, (draft) => {
              ...
              if (childValue !== value) {
                if (target.$_mutable) {
                  ...
                  target.next(fn(target.value, (draft) => {
                    setKey(draft, key, value);
                  }));
                }
              }
    ...
    var cleanMixin = (BaseClass) => class WithClean extends BaseClass {
      constructor(val, config, ...args) {
        super(val, config, ...args);
    ...
    class Mirror$1 extends childMixin(cleanMixin(propsMixin(actionMixin(eventMixin(BehaviorSubject))))) {
      constructor(value, config = {}) {
        super(get_1(config, 'mutable') ? value : asImmer(value), config);
        this.$_config(config);
      }
      ...
      $_config(config) {
        ...
        this.$name = isStr(name, true) ? name : id();
        this.$_debug = debug;
      ...
      $update(values, offset = 0) {
        ...
        try {
          const next = fn(this.value, (draft) => {
            switch (type) {
              case TYPE_MAP:
                values.forEach((keyValue, key) => {
    ...
    Object.assign(Mirror$1.prototype, eventMixin);

    setInstance(Mirror$1);
    var index = {
      ...constants, ...idGen, idGen, utils, Mirror: Mirror$1, isMirror, create
    };
