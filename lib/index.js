(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    function mapFromKeys$1(target, keyMap) {
    ...
    class Mirror extends BehaviorSubject {
      ...
      get $_lastQueuedValue() {
        if (!this.$_valueQueue.isStopped) {
          let index = this.$_valueQueue.value.length - 1;
          while (index >= 0) {
            const queueSub = this.$_valueQueue.value[index];
      ...
      get $_pendingValue() {
        ...
        if (queuedValue !== ABSENT) return queuedValue;
       ...
       */
      get $_valueQueue() {
        return lazy(this, '$__valueQueue', (target) => {
          const queue = new BehaviorSubject([]);
          // @TODO: pipe with distinct
          queue.pipe(
            switchMap((list) => from(list)),
              distinct()
          ).subscribe({
            next(subject) {
                 target.$_processNewCandidate(subject);
              }
          });
          return queue;
        });
      ...
      $_processNewCandidate(candidate) {
        if ((!candidate) || candidate.isStopped) {
          return;
        }
        const target = this;
        const childMapSubs = candidate.pipe(
          map(toMap),
          map((valueMap) => {
            return mapFromKeys(valueMap, target.$children);
          })
        );
      }
       ...
       */
      $_shardSubject(subject) {
        if (subject.isStopped || (!this.$isContainer)) return;
        const target = this;
        subject.subscribe({
          next(subjectValue) {
            target.$children.forEach((child, childName) => {
            });
          }
        });
      ...
      $_flushSubFromValueQueue(sub, nextValue = ABSENT) {
        if (!this.$_valueQueue.value.includes(sub)) {
          return;
        }
        if (nextValue !== ABSENT) {
          super.next(nextValue);
