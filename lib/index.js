(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    const PHASE_DEFAULT_LIST = [PHASE_INIT, PHASE_PRE, PHASE_ON, PHASE_POST];
        ...
        this.$events.subscribe({
          next(evt) {
            if (evt && evt.subscribe) {
              evt.subscribe({
                error(err) {
                  console.log('error in mirror: ', get_1(self, 'name', self), ':', get_1(err, 'message', err));
                  if (err && err.thrown) {
    ...
    class MirrorCollection extends Mirror {
      ...
      $updateChild(key, value) {
        ...
        if (!isEqual_1(child.value, value)) {
        ...
        const watchingKeys = this.pipe(
          ...
          distinctUntilChanged((itemA, itemB) => {
            for (let i = 0; i < keys.length; ++i) {
              ...
              if (itemA.has(keys[i]) && !isEqual_1(itemA.get(keys[i]), itemB.get(keys[i]))) {
                return false;
              }
            }
            return true;
