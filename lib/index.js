    ...
    ***************************************************************************** */
    ...
    var withTrans = (BaseClass) => (class WithTrans extends BaseClass {
      ...
      $_addTrans(def) {
        ...
        const trans = fn(def, (draft) => MirrorTrans.make(draft));
     ...
     */
    class Mirror$1 extends (withChildren(withEvents(withTrans(BehaviorSubject)))) {
      ...
      next(nextValue) {
        if (!this.$_constructed) {
          ...
          if (get_1(currentTrans, 'errors.length', 0) > 0) {
