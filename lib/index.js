(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    const EVENT_TYPE_ACCEPT_FROM = 'event:acceptFrom';
    const EVENT_TYPE_REMOVE_FROM = 'event:removeFrom';

    const EVENT_TYPE_FLUSH_ACTIVE = 'event:commit';
    const EVENT_TYPE_ACTION = 'event:action';
    const EVENT_TYPE_MUTATE = 'event:mutate';
    const EVENT_TYPE_VALIDATE = 'event:validate';
    const SET_RE = /^set(.+)$/i;

    const MIRROR_EVENT_STATE_ACTIVE = 'mirror-event-state:active';
    const MIRROR_EVENT_STATE_ERROR = 'mirror-state:error';
    const MIRROR_EVENT_STATE_STOPPED = 'mirror-state:stopped';
    const MIRROR_EVENT_STATE_COMPLETE = 'mirror-state:stopped';
          ...
          next(value) {
            if (target.$lastChange) {
              ...
              if (lastValueChildValue !== value) {
                target.$lastChange._value = fn(target.$lastChange.value, (draft) => {
    ...
    class Mirror extends childMixin(propsMixin(actionMixin(eventMixin(BehaviorSubject)))) {
    ...
    var index = {
      ...constants, ...idGen, idGen, utils, Mirror,
    };
