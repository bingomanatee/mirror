(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    var withTrans = (BaseClass) => (class WithTrans extends BaseClass {
      ...
      $commit() {
        ...
        if (this.$_pending.value.length) {
          const last = sortBy_1(this.$_pending.value, 'order').pop();
          this.$_pending.next([]);
      ...
      $_addTrans(def) {
        ...
        const mirror = this;
        const trans = def instanceof MirrorTrans ? def : fn(def, (draft) => MirrorTrans.make(mirror, draft));
      ...
      $_addErrorsToTrans(matchTo, errors) {
        const uinqErrors = uniq_1(errors)
          .filter((e) => e);
        if (uinqErrors.length > 0) {
          this.$_updateTrans(matchTo, (draft) => {
            draft.errors = uniq_1([...draft.errors, ...uinqErrors])
              .filter((e) => e);
          });
        }
      }
      ...
      constructor(...init) {
        ...
        this.$on(
          ...
          ({
            ...
            try {
              const nextValue = fn(t.value, (draft) => fn$1(draft, t, ...args));
              t.next(nextValue);
            } catch (err) {
              p.error(err);
            }
    ...
    function setMirrorClass(MirrorClass) {
      Mirror = MirrorClass;
    }
    ...
    var withEvents = (BaseClass) => class WithEvents extends BaseClass {
      constructor(...args) {
        super(...args);
     ...
     */
    class Mirror$1 extends withAction(withChildren(withEvents(withTrans(BehaviorSubject)))) {
      ...
      $keys() {
        ...
        return uniq_1([...valueKeys, ...childKeys])
          .sort();
      }
    ...
    setMirrorClass(Mirror$1);
    var index = {
      ...constants, ...idGen, idGen, utils, Mirror: Mirror$1, mirrorWatcher, MirrorTrans,
    };
