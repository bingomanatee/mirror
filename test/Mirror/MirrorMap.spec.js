...
const { TYPE_MAP, utils, CHILDREN } = lib;

function isOdd(n) {
  return utils.isNumber(n) ? (!!(n % 2)) : false;
}
...
tap.test(p.name, (suite) => {
  suite.test(`${subjectName} (Map type)`, (suiteTests) => {
    ...
    suiteTests.test('tests', (t) => {
        ...
        tests: new Map([
          [
            'x', (value) => {
              ...
              if (isOdd(x)) {
                console.log('--- x is odd: ', x);
                throw new Error('x must be even');
              }
              console.log('--- ', x, 'is even');
            },
          ],
          [CHILDREN, (value) => (utils.isNumber(value) ? false : 'value must be a number')],
        ]),
      });
      const [{ history, errors }] = watch(m);
      ...
      t.equal(m.$get('y'), 4);
      t.equal(m.$get('z'), 3);
      t.same(m.value, new Map([['x', 0], ['y', 4], ['z', 3]]));
      ...
      t.same(history.length, 4); // one for each child change; will ultimately be 3 for transactions
      t.same(errors.length, 0);
